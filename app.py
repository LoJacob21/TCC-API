...
# Forçar IPv4 (Render aparentemente não resolve IPv6 do Supabase)
original_getaddrinfo = socket.getaddrinfo
def getaddrinfo_ipv4(*args, **kwargs):
    return [x for x in original_getaddrinfo(*args, **kwargs) if x[0] == socket.AF_INET]
...
def listar_leituras():
    ...
    if periodo == "1d":
        ...
        return abort(400, "Período inválido! Use 1d, 7d ou 30d.")
        
    """Retorna todas as leituras dos sensores"""
    try:
...
def listar_imagens():
    """Retorna imagens salvas filtradas por período"""
    periodo = request.args.get('periodo', default='1d')  # padrão: últimas 24h
    agora = datetime.now(timezone.utc)
    ...
    if periodo == '1d':
        ...
        return abort(400, "Parâmetro 'periodo' inválido. Use 1d, 7d ou 30d.")
...
def upload_imagem():
    """Endpoint para upload de imagens via HTTP (multipart/form-data ou raw)"""
    try:
        ...
        if 'image' in request.files:
            ...
            # Upload via raw data (binário direto)
            image_data = request.data
            if len(image_data) == 0:
                return jsonify({"erro": "Nenhum dado de imagem recebido"}), 400
            
            # Verifica se é um JPEG válido
            if len(image_data) > 10 and image_data[0] == 0xFF and image_data[1] == 0xD8:
...
# ===============================
# Configuração MQTT (apenas para sensores)
# ===============================
...
def mqtt_worker():
    ...
    try:
        ...
        mqtt_worker()  # Recursão para reconexão
...
if __name__ == '__main__':
    ...
    print(f"  POST /api/upload       -> Upload de imagens (form-data ou raw)")
    ...
    print("=" * 60)
    print("Upload de imagens suporta:")
    print("  1. Multipart/form-data: campo 'image'")
    print("  2. Raw data: binário JPEG direto no body")
    print("=" * 60)
